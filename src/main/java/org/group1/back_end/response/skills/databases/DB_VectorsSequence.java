...
public class DB_VectorsSequence implements iDataBase{
    ...
    public void add(String key, String value) {
        ...
        QUERY_VECTOR_MATCHING.put(process(key), value);
    ...
    public static double[][] sinkhorn(double[][] costMatrix, double[] source, double[] target, double regularization, double epsilon) {
        int n = source.length;
        int m = target.length;
        double[][] p = new double[n][m];
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < m; j++) {
                p[i][j] = Math.exp(-costMatrix[i][j] / regularization);
            }
        }
        ...
        while (error > epsilon) {
            ...
            for (int i = 0; i < n; i++) {
                ...
                error = Math.max(error, Math.abs(sum - source[0]));
            }
        }
        return p;
    }
